<template>
	<div :class="['text-xs-center', 'middle-box', $vuetify.breakpoint.width > 500 ? ['elevation-4', 'mt-5'] : '' ]" transition="scale-transition" origin="center center">
        ...
        <p style="color: #378FB0">Preencha os dados abaixo e faça sua solicitação de acesso.</p>
...
export default {
	...
	data: () => ({
		...
		passwordRules: [
			...
			(v) => (v && v.length >= 8) || 'Pelo menos 8 caracteres.'
		],
		captcha: false,
    	e1: true
	}),
	methods: {
		...
		async submit () {
			if (!this.captcha) {
				...
				const { nome, cpf, email, password } = this
				const response = await http.put('/auth/users', {nome, cpf, email, senha: password})
				if (response.status === 200) {
					bus.$emit('display-alert', {
						type: 'success',
						message: 'Solicitação realizada! Aguarde a liberação do acesso.'
					})				
				} else {
					bus.$emit('display-alert', {
						type: 'error',
